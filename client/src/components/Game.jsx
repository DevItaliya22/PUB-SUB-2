...
function Game() {
  ...
  // Sample users data
  const users = [
    { username: 'User1', mode: 'player', count: 3 },
    { username: 'User2', mode: 'spectator', count: 0 },
    { username: 'User3', mode: 'player', count: 5 },
  ];
  ...
  //this use effect will run only once when the component is mounted
  useEffect(() => {
    const Socket = io("http://localhost:3000");
    
    Socket.on("connect", () => {
      console.log(gameid, Socket.id, mode); // Socket.id should be available here after connection
